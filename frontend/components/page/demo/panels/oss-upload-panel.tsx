...
const OSS_PREVIEW_PREFIX = (process.env.OSS_PREVIEW_PREFIX as string | undefined) ?? "";
...
function buildPreviewUrl(prefix: string, key: string) {
...
export function OssUploadPanel() {
	...
	async function handleUpload(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const isImage = IMAGE_EXTENSIONS.includes(extension as (typeof IMAGE_EXTENSIONS)[number]);
			...
			setUploadResult({
				...
				directUrl: buildPublicUrl(currentSignature.host, key),
				previewUrl: buildPreviewUrl(OSS_PREVIEW_PREFIX, key),
				extension,
				isImage,
				statusCode: response.status,
			});
		...
		<div className='space-y-6'>
			...
			{uploadStatus === "success" && uploadResult ? (
				<section className='rounded-[24px] border border-[rgba(35,118,79,0.18)] bg-[rgba(35,118,79,0.08)] p-5'>
					<p className='text-sm uppercase tracking-[0.16em] text-[rgba(35,86,62,0.95)]'>Upload success</p>
					<div className='mt-4 space-y-2 text-sm leading-7 text-[rgba(35,86,62,0.95)]'>
						<p>statusCode: {uploadResult.statusCode}</p>
						<p className='text-safe-wrap'>previewPath: OSS_PREVIEW_PREFIX + key</p>
						<p className='text-safe-wrap'>host: {uploadResult.host}</p>
						<p className='text-safe-wrap'>key: {uploadResult.key}</p>
						<p className='text-safe-wrap'>previewUrl: {uploadResult.previewUrl}</p>
						<p className='text-safe-wrap'>directUrl: {uploadResult.directUrl}</p>
					</div>
					{uploadResult.isImage ? (
						<div className='mt-4 rounded-[16px] border border-black/10 bg-white/75 p-3'>
							<img
								src={uploadResult.previewUrl}
								alt={uploadResult.key}
								className='h-auto max-h-[380px] w-full rounded-[12px] object-contain'
							/>
						</div>
					) : null}
				</section>
